...
function getsalary($id)
{
    ...
    //return ['status' => 1, 'todaysalary' => $todaysalary + $invite_bonus['invite_today'], 'salary' => $salary + $invite_bonus['invite_month']];//今日提成，本月提成
	
    return ['status' => 1, 'todaysalary' => $todaysalary, 'salary' => $salary ];//今日提成，本月提成，佣金不算在提成里面
}


function get_invite_bonus($job_number)//佣金
{
